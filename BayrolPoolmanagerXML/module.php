...
class BayrolPoolmanagerXML extends IPSModule
{
    ...
    public function RunDiscoveryBatch()
    {
        ...
        $this->SetValueByIdent('DiscoveryCSV', BPMXML_DiscoveryExporter::toCsv($results));
    ...
    private function ClassifyDiscoveryEntry($a) { return BPMXML_DiscoveryClassifier::classify($a); }
    private function MergeDiscoveryDB($results) { return BPMXML_DiscoveryDatabase::merge($this->GetValueByIdent('DiscoveryDB'), $results); }
    private function BuildDiscoveryReport($results, $db, $scanned, $valid, $errors) { return BPMXML_DiscoveryDatabase::report($results, $db, (int)$scanned, (int)$valid, (int)$errors); }
    private function BuildSnapshotCompareText($a, $b) { return BPMXML_SnapshotComparator::compare($a, $b); }
    private function GenerateDefinitionText($data) { return BPMXML_DiscoveryExporter::toPhpDefinition($data); }
    private function RunExplorerRange($type, $start, $end, $title) { if (trim($this->ReadPropertyString('Host')) === '') { $this->SetStatus(201); return false; } if ($end < $start) { $tmp = $start; $start = $end; $end = $tmp; } $max = max(1, min($this->ReadPropertyInteger('ExplorerMaxPerRun'), 1000)); $onlyValid = $this->ReadPropertyBoolean('ExplorerOnlyValid'); $storeRaw = $this->ReadPropertyBoolean('ExplorerStoreRaw'); $results = []; $scanned = 0; $valid = 0; for ($id = $start; $id <= $end && $scanned < $max; $id++) { $entry = $this->ScanXmlItem($type, $id, $storeRaw); $scanned++; if (!empty($entry['valid'])) { $valid++; } if (!$onlyValid || !empty($entry['valid'])) { $results[] = $entry; } } $summary = $title . ': Typ ' . $type . ', Bereich ' . $start . '-' . $end . ', abgefragt ' . $scanned . ', gueltig ' . $valid . ', gespeichert ' . count($results) . ', Zeit ' . date('Y-m-d H:i:s'); $this->SetValueByIdent('ExplorerSummary', $summary); $this->SetValueByIdent('ExplorerCSV', BPMXML_DiscoveryExporter::toCsv($results, true, false)); $this->SetValueByIdent('ExplorerJSON', json_encode($results, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)); $this->SetValueByIdent('ExplorerLastRun', time()); return true; }
