...
class BayrolPoolmanagerXML extends IPSModule
{
    ...
    {
        $typeStart = $this->ReadPropertyInteger('DiscoveryTypeStart');
        $typeEnd = $this->ReadPropertyInteger('DiscoveryTypeEnd');
        $idStart = $this->ReadPropertyInteger('DiscoveryIdStart');
        $idEnd = $this->ReadPropertyInteger('DiscoveryIdEnd');
        return $this->RunDiscoveryRange($typeStart, $typeEnd, $idStart, $idEnd, 'Discovery');
    }

    public function StoreDiscoverySnapshotA()
    {
        $this->RunDiscovery();
        $this->SetValueByIdent('DiscoverySnapshotA', $this->GetValueByIdent('DiscoveryJSON'));
        $this->SetValueByIdent('DiscoveryCompare', 'Discovery Snapshot A gespeichert: ' . date('Y-m-d H:i:s'));
        return true;
    }

    public function StoreDiscoverySnapshotB()
    {
        $this->RunDiscovery();
        $this->SetValueByIdent('DiscoverySnapshotB', $this->GetValueByIdent('DiscoveryJSON'));
        $this->SetValueByIdent('DiscoveryCompare', 'Discovery Snapshot B gespeichert: ' . date('Y-m-d H:i:s'));
        return true;
    }

    public function CompareDiscoverySnapshots()
    {
        $a = json_decode($this->GetValueByIdent('DiscoverySnapshotA'), true);
        $b = json_decode($this->GetValueByIdent('DiscoverySnapshotB'), true);
        if (!is_array($a) || !is_array($b)) {
            $this->SetValueByIdent('DiscoveryCompare', 'Discovery Snapshot A oder B fehlt / ist kein gueltiges JSON.');
            return false;
        }
        $this->SetValueByIdent('DiscoveryCompare', $this->BuildSnapshotCompareText($a, $b));
        return true;
    }
    ...
    public function CompareSnapshots()
    {
        ...
        $this->SetValueByIdent('ExplorerCompare', $this->BuildSnapshotCompareText($a, $b));
        return true;
    }
    ...
    public function GenerateDefinitionFromExplorer()
    {
        ...
        $this->SetValueByIdent('ExplorerDefinition', $this->GenerateDefinitionText($data));
        return true;
    }

    public function ClearExplorer()
    {
        $ids = ['ExplorerSummary','ExplorerCSV','ExplorerJSON','ExplorerLastRun','ExplorerSnapshotA','ExplorerSnapshotB','ExplorerCompare','ExplorerDefinition','DiscoverySummary','DiscoveryCSV','DiscoveryJSON','DiscoveryDB','DiscoverySnapshotA','DiscoverySnapshotB','DiscoveryCompare','DiscoveryReport','DiscoveryLastRun'];
        foreach ($ids as $ident) { $this->SetValueByIdent($ident, strpos($ident, 'LastRun') !== false ? 0 : ''); }
    }

    public function RequestAction($Ident, $Value)
    {
        if (!$this->IsWritePrepared($Ident)) { throw new Exception('Schreibzugriff fuer ' . $Ident . ' ist nicht freigegeben.'); }
    ...
    private function RunDiscoveryRange($typeStart, $typeEnd, $idStart, $idEnd, $title)
    {
        ...
        $results = [];
        $csv = "xmlitem;class;confidence;valid;label;unit;value;active;displayed;attributes;duration_ms;error\n";
        $scanned = 0; $valid = 0; $errors = 0;

        for ($type = $typeStart; $type <= $typeEnd && $scanned < $max; $type++) {
            for ($id = $idStart; $id <= $idEnd && $scanned < $max; $id++) {
        ...
        $summary = $title . ': Typ ' . $typeStart . '-' . $typeEnd . ', ID ' . $idStart . '-' . $idEnd . ', abgefragt ' . $scanned . ', gueltig ' . $valid . ', gespeichert ' . count($results) . ', Zeit ' . date('Y-m-d H:i:s');
    ...
    private function ScanXmlItem($type, $id, $storeRaw)
    {
        $started = microtime(true);
        $entry = ['xmlitem' => $type . '.' . $id, 'valid' => false, 'attributes' => [], 'duration_ms' => 0, 'error' => '', 'class' => 'invalid', 'confidence' => 0];
        try {
            $raw = $this->FetchRawXml($type, $id);
            $entry['duration_ms'] = round((microtime(true) - $started) * 1000, 1);
            $xml = $this->ParseXmlString($raw);
            if (!isset($xml->item)) { throw new Exception('XML enthaelt kein item'); }
            foreach ($xml->item->attributes() as $key => $value) { $entry['attributes'][(string)$key] = (string)$value; }
            $entry['valid'] = true;
            $class = $this->ClassifyDiscoveryEntry($entry['attributes']);
            $entry['class'] = $class['class'];
            $entry['confidence'] = $class['confidence'];
    ...
    private function ClassifyDiscoveryEntry($a)
    {
        $label = strtolower((string)($a['label'] ?? ''));
        $unit = (string)($a['unit'] ?? '');
        $value = (string)($a['value'] ?? '');
        if (isset($a['active']) || isset($a['displayed'])) { return ['class' => 'alarm', 'confidence' => 95]; }
        if (strpos($label, 'status') !== false) { return ['class' => 'status', 'confidence' => 90]; }
        if (strpos($label, 'betriebsart') !== false) { return ['class' => 'operating_mode', 'confidence' => 90]; }
        foreach (['out ', 'relais', 'licht', 'pumpe', 'filter', 'heizung', 'solar', 'eco', 'flock', 'salz', 'elektrolyse', 'ventil'] as $kw) {
            if (strpos($label, $kw) !== false && $unit === '' && ($value === '0' || $value === '1')) { return ['class' => 'output_status', 'confidence' => 88]; }
        }
        if (strpos($label, 'alarm') !== false || strpos($label, 'grenze') !== false) { return ['class' => 'limit', 'confidence' => 80]; }
        if (strpos($label, 'soll') !== false || strpos($label, 'setpoint') !== false) { return ['class' => 'setpoint', 'confidence' => 80]; }
        if (strpos($label, 'kalib') !== false || preg_match('/(ph|mv|cl).*\d/', $label)) { return ['class' => 'calibration', 'confidence' => 70]; }
        if (in_array($unit, ['pH', 'mV', 'mg/l', 'µA', 'mA', '°C', 'C', 'V', 'l', 'min', '%', 'mS/cm'], true)) { return ['class' => 'measurement', 'confidence' => 75]; }
        if ($unit === '' && is_numeric($value)) { return ['class' => 'numeric_status_or_config', 'confidence' => 50]; }
        return ['class' => 'unknown', 'confidence' => 20];
    }

    private function MergeDiscoveryDB($results)
    {
        $db = json_decode($this->GetValueByIdent('DiscoveryDB'), true);
        if (!is_array($db)) { $db = []; }
        $now = date('Y-m-d H:i:s');
        foreach ($results as $entry) {
            if (empty($entry['valid'])) { continue; }
            $key = $entry['xmlitem'];
            if (!isset($db[$key])) { $db[$key] = ['xmlitem' => $key, 'first_seen' => $now, 'scan_count' => 0]; }
    ...
    private function BuildDiscoveryReport($results, $db, $scanned, $valid, $errors)
    {
        $classes = [];
        foreach ($results as $entry) { $c = $entry['class'] ?? 'unknown'; $classes[$c] = ($classes[$c] ?? 0) + 1; }
        ksort($classes);
        $lines = [];
        ...
        $lines[] = 'Discovery-DB Eintraege gesamt: ' . count($db);
        $lines[] = '';
        $lines[] = 'Klassen im aktuellen Lauf:';
        foreach ($classes as $class => $count) { $lines[] = '- ' . $class . ': ' . $count; }
        return implode("\n", $lines);
    }

    private function BuildSnapshotCompareText($a, $b)
    {
        $ma = $this->MapExplorerResult($a); $mb = $this->MapExplorerResult($b);
        $relevant = []; $ignored = []; $all = [];
        foreach ($mb as $xmlitem => $entryB) {
            if (!isset($ma[$xmlitem])) { $line = '+ ' . $this->FormatExplorerEntry($xmlitem, [], $entryB['attributes'] ?? []); $relevant[] = $line; $all[] = $line; continue; }
            $oldAttr = $ma[$xmlitem]['attributes'] ?? []; $newAttr = $entryB['attributes'] ?? [];
            if (json_encode($oldAttr) === json_encode($newAttr)) { continue; }
            $line = '* ' . $this->FormatExplorerEntry($xmlitem, $oldAttr, $newAttr); $all[] = $line;
            if ($this->IsRelevantSwitchChange($oldAttr, $newAttr)) { $relevant[] = $line; } else { $ignored[] = $line; }
        }
        foreach ($ma as $xmlitem => $entryA) { if (!isset($mb[$xmlitem])) { $line = '- ' . $this->FormatExplorerEntry($xmlitem, $entryA['attributes'] ?? [], []); $relevant[] = $line; $all[] = $line; } }
        $text = ['Relevante Status-/Schalt-Aenderungen: ' . count($relevant), 'Ignorierte Messwert-Drift: ' . count($ignored), 'Alle Aenderungen: ' . count($all), ''];
        if (count($relevant) > 0) { $text[] = '--- RELEVANT ---'; $text[] = implode("\n", $relevant); } else { $text[] = 'Keine relevanten Status-/Schalt-Aenderungen erkannt.'; }
        if (count($ignored) > 0) { $text[] = ''; $text[] = '--- IGNORIERT: Messwert-Drift / analoge Werte ---'; $text[] = implode("\n", array_slice($ignored, 0, 80)); if (count($ignored) > 80) { $text[] = '... weitere ignorierte Aenderungen: ' . (count($ignored) - 80); } }
        return implode("\n", $text);
    }

    private function GenerateDefinitionText($data)
    {
        $out = "[\n";
        foreach ($data as $entry) {
            if (empty($entry['valid']) || empty($entry['attributes'])) { continue; }
            $a = $entry['attributes']; $label = $a['label'] ?? $entry['xmlitem']; $unit = $a['unit'] ?? ''; $profile = $this->SuggestProfile($unit, $a); $ident = $this->MakeIdent($label, $entry['xmlitem']);
    ...
    private function RunExplorerRange($type, $start, $end, $title)
    {
        ...
        $max = max(1, min($this->ReadPropertyInteger('ExplorerMaxPerRun'), 1000)); $onlyValid = $this->ReadPropertyBoolean('ExplorerOnlyValid'); $storeRaw = $this->ReadPropertyBoolean('ExplorerStoreRaw');
        $results = []; $csv = "xmlitem;valid;label;unit;value;active;displayed;attributes;duration_ms;error\n"; $scanned = 0; $valid = 0;
        for ($id = $start; $id <= $end && $scanned < $max; $id++) {
            $entry = $this->ScanXmlItem($type, $id, $storeRaw); $scanned++; if (!empty($entry['valid'])) { $valid++; }
            if (!$onlyValid || !empty($entry['valid'])) { $results[] = $entry; $a = $entry['attributes']; $csv .= $this->Csv($entry['xmlitem']) . ';' . ($entry['valid'] ? '1' : '0') . ';' . $this->Csv($a['label'] ?? '') . ';' . $this->Csv($a['unit'] ?? '') . ';' . $this->Csv($a['value'] ?? '') . ';' . $this->Csv($a['active'] ?? '') . ';' . $this->Csv($a['displayed'] ?? '') . ';' . $this->Csv(json_encode($a, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)) . ';' . $this->Csv((string)$entry['duration_ms']) . ';' . $this->Csv($entry['error']) . "\n"; }
        }
        $summary = $title . ': Typ ' . $type . ', Bereich ' . $start . '-' . $end . ', abgefragt ' . $scanned . ', gueltig ' . $valid . ', gespeichert ' . count($results) . ', Zeit ' . date('Y-m-d H:i:s');
        $this->SetValueByIdent('ExplorerSummary', $summary); $this->SetValueByIdent('ExplorerCSV', $csv); $this->SetValueByIdent('ExplorerJSON', json_encode($results, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES)); $this->SetValueByIdent('ExplorerLastRun', time()); return true;
    }
    ...
    private function UpdateInternal($staticOnly)
    {
        ...
        $ok = 0; $errors = [];
        if (!$staticOnly) {
            foreach (self::ITEMS as $item) { if ($item['kind'] === 'setpoint' && !$this->ReadPropertyBoolean('ReadSetpoints')) { continue; } if ($this->ReadValueItem($item, $errors)) { $ok++; } }
            if ($this->ReadPropertyBoolean('ReadAlarms')) { foreach (self::ALARMS as $id => $label) { if ($this->ReadAlarmItem($id, $label, $errors)) { $ok++; } } }
            if ($this->ReadPropertyBoolean('ReadAlarmList')) { try { $this->SetValueByIdent('AlarmList', $this->ReadAlarmListText()); $ok++; } catch (Exception $e) { $errors[] = '1.1092: ' . $e->getMessage(); } }
        }
        $this->SetValueByIdent('Online', $ok > 0 || $staticOnly); $this->SetValueByIdent('SuccessfulReads', $ok); $this->SetValueByIdent('LastUpdate', time()); $this->SetValueByIdent('LastError', implode("\n", array_slice($errors, 0, 40))); $this->SetValueByIdent('WriteMode', $this->ReadPropertyBoolean('EnableWritePreparation'));
        if ($ok > 0 || $staticOnly) { $this->SetStatus(102); return true; } $this->SetStatus(202); return false;
    }

    private function ReadValueItem($definition, &$errors)
    {
        try { list($type, $id) = explode('.', $definition['xml']); $item = $this->ReadItem((int)$type, (int)$id); if (!array_key_exists('value', $item)) { throw new Exception('Attribut value fehlt'); } $this->SetValueByIdent($definition['ident'], (float)$item['value']); return true; } catch (Exception $e) { $errors[] = $definition['xml'] . ' ' . $definition['name'] . ': ' . $e->getMessage(); return false; }
    }

    private function ReadAlarmItem($id, $label, &$errors)
    {
        try { $item = $this->ReadItem(44, $id); $this->SetValueByIdent('AlarmActive' . $id, isset($item['active']) && ((int)$item['active']) === 1); $this->SetValueByIdent('AlarmDisplayed' . $id, isset($item['displayed']) && ((int)$item['displayed']) === 1); return true; } catch (Exception $e) { $errors[] = '44.' . $id . ' ' . $label . ': ' . $e->getMessage(); return false; }
    }
    ...
    {
        $pos = 10;
        foreach (self::ITEMS as $item) { if ($item['kind'] === 'setpoint' && !$this->ReadPropertyBoolean('ReadSetpoints')) { continue; } $cat = $this->GetCategoryID($item['category'], self::CATEGORIES[$item['category']]['name'], self::CATEGORIES[$item['category']]['pos']); $this->RegisterFloat($cat, $item['ident'], $item['name'], $item['profile'], $pos++); }
        if ($this->ReadPropertyBoolean('ReadAlarms')) { $cat = $this->GetCategoryID('Alarms', 'Alarme', 50); $pos = 10; foreach (self::ALARMS as $id => $label) { $this->RegisterBoolean($cat, 'AlarmActive' . $id, $label . ' aktiv', 'BPMXML.Alarm', $pos++); $this->RegisterBoolean($cat, 'AlarmDisplayed' . $id, $label . ' angezeigt', 'BPMXML.Alarm', $pos++); } $this->RegisterString($cat, 'AlarmList', 'Alarmuebersicht', '', 900); }
    }

    private function RegisterServiceVariables()
    {
        $cat = $this->GetCategoryID('Service', 'Service', 900); $this->RegisterBoolean($cat, 'Online', 'Kommunikation online', 'BPMXML.Online', 10); $this->RegisterInteger($cat, 'SuccessfulReads', 'Erfolgreiche XML-Abfragen', '', 20); $this->RegisterInteger($cat, 'LastUpdate', 'Letzte Aktualisierung', '~UnixTimestamp', 30); $this->RegisterString($cat, 'LastError', 'Letzte Fehler / uebersprungene Adressen', '', 40); $this->RegisterBoolean($cat, 'WriteMode', 'Schreibmodus vorbereitet', 'BPMXML.WriteMode', 50);
    }

    private function RegisterExplorerVariables()
    {
        $cat = $this->GetCategoryID('Explorer', 'XML Explorer', 800); $this->RegisterString($cat, 'ExplorerSummary', 'Scan-Zusammenfassung', '', 10); $this->RegisterString($cat, 'ExplorerCSV', 'Scan-Ergebnis CSV', '', 20); $this->RegisterString($cat, 'ExplorerJSON', 'Scan-Ergebnis JSON', '', 30); $this->RegisterInteger($cat, 'ExplorerLastRun', 'Letzter Explorer-Lauf', '~UnixTimestamp', 40); $this->RegisterString($cat, 'ExplorerSnapshotA', 'Snapshot A JSON', '', 50); $this->RegisterString($cat, 'ExplorerSnapshotB', 'Snapshot B JSON', '', 60); $this->RegisterString($cat, 'ExplorerCompare', 'Snapshot Vergleich', '', 70); $this->RegisterString($cat, 'ExplorerDefinition', 'PHP-Definition Vorschlag', '', 80);
    }

    private function RegisterDiscoveryVariables()
    {
        $cat = $this->GetCategoryID('Discovery', 'Discovery Engine', 850); $this->RegisterString($cat, 'DiscoverySummary', 'Discovery Zusammenfassung', '', 10); $this->RegisterString($cat, 'DiscoveryCSV', 'Discovery CSV', '', 20); $this->RegisterString($cat, 'DiscoveryJSON', 'Discovery JSON aktueller Lauf', '', 30); $this->RegisterString($cat, 'DiscoveryDB', 'Discovery Datenbank JSON', '', 40); $this->RegisterString($cat, 'DiscoverySnapshotA', 'Discovery Snapshot A JSON', '', 50); $this->RegisterString($cat, 'DiscoverySnapshotB', 'Discovery Snapshot B JSON', '', 60); $this->RegisterString($cat, 'DiscoveryCompare', 'Discovery Snapshot Vergleich', '', 70); $this->RegisterString($cat, 'DiscoveryReport', 'Discovery Report', '', 80); $this->RegisterInteger($cat, 'DiscoveryLastRun', 'Letzter Discovery-Lauf', '~UnixTimestamp', 90);
    }

    private function RegisterProfiles()
    {
        $this->CreateFloatProfile('BPMXML.pH', 'pH', 2); $this->CreateFloatProfile('BPMXML.mg_l', 'mg/l', 2); $this->CreateFloatProfile('BPMXML.mV', 'mV', 0); $this->CreateFloatProfile('BPMXML.C', ' C', 1); $this->CreateFloatProfile('BPMXML.V', 'V', 2); $this->CreateFloatProfile('BPMXML.l', 'l', 1); $this->CreateFloatProfile('BPMXML.percent', '%', 0); $this->CreateFloatProfile('BPMXML.min', 'min', 0); $this->CreateFloatProfile('BPMXML.microA', 'µA', 2); $this->CreateFloatProfile('BPMXML.mA', 'mA', 1); $this->CreateFloatProfile('BPMXML.mScm', 'mS/cm', 1); $this->CreateBoolProfile('BPMXML.Alarm', 'OK', 'Alarm', 0x00AA00, 0xFF0000); $this->CreateBoolProfile('BPMXML.Online', 'Offline', 'Online', 0xFF0000, 0x00AA00); $this->CreateBoolProfile('BPMXML.WriteMode', 'Read-only', 'Freigegeben', 0x00AA00, 0xFFA500);
    }
    ...
    private function ReadItem($type, $id) { $xml = $this->FetchXml($type, $id); if (!isset($xml->item)) { throw new Exception('XML enthaelt kein item'); } $result = []; foreach ($xml->item->attributes() as $key => $value) { $result[(string)$key] = (string)$value; } return $result; }
    ...
    private function FetchRawXml($type, $id) { $host = preg_replace('#^https?://#', '', trim($this->ReadPropertyString('Host'))); $url = 'http://' . $host . '/cgi-bin/webgui.fcgi?xmlitem=' . $type . '.' . $id; $context = stream_context_create(['http' => ['method' => 'GET', 'timeout' => $this->ReadPropertyInteger('Timeout'), 'ignore_errors' => true, 'header' => "Connection: close\r\nUser-Agent: IP-Symcon BayrolPoolmanagerXML\r\n"]]); $raw = @file_get_contents($url, false, $context); if ($raw === false || trim($raw) === '') { throw new Exception('keine HTTP-Antwort'); } return trim($raw); }
    private function ParseXmlString($raw) { $xmlStart = strpos($raw, '<'); if ($xmlStart === false) { throw new Exception('Antwort ist kein XML: ' . substr($raw, 0, 100)); } if ($xmlStart > 0) { $raw = substr($raw, $xmlStart); } if ($this->ReadPropertyBoolean('DebugXml')) { $this->SendDebug('XML', substr($raw, 0, 500), 0); } $xml = @simplexml_load_string($raw); if (!$xml instanceof SimpleXMLElement) { throw new Exception('ungueltiges XML: ' . substr($raw, 0, 120)); } return $xml; }
